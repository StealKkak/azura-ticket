...
async def initDB():
    ...
    await cur.execute("CREATE TABLE IF NOT EXISTS ticket_settings (id INTEGER PRIMARY KEY AUTOINCREMENT, guild TEXT, name TEXT, survey1 TEXT, survey2 TEXT, survey3 TEXT, role TEXT, user_close INTEGER NOT NULL DEFAULT 0, dup_ticket INTEGER NOT NULL DEFAULT 0, ticket_category TEXT, closed_ticket_category TEXT)")
    await cur.execute("CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, guilds TEXT, refresh_token TEXT)")
    await cur.execute("CREATE TABLE IF NOT EXISTS usernames (id TEXT PRIMARY KEY, name TEXT)")
    await con.commit()
