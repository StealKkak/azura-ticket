...
async def createTicket(interaction: discord.Interaction, ticketTypeId, answer1 = None, answer2 = None, answer3 = None):
    ...
    embed = makeEmbed("info", ticketType.name, "티켓이 생성되었습니다!" if not ticketType.embed else ticketType.embed)
                        ...
                        try:
                            ...
                            if mInteraction.data["custom_id"] == "save": #티켓 저장 여부: 참
                                ...
                                try:
                                    ...
                                    await user.send(embed=embed, view=discord.ui.View().add_item(discord.ui.Button(style=discord.ButtonStyle.url, label="대화내용 보기", url=f"{domain}/tickets/{interaction.guild.id}/{interaction.channel.id}")))
                                except discord.NotFound:
                                    pass
                                except:
                                    traceback.print_exc()
