...
from azura_chat_exporter.construct.message import gather_messages
from azura_chat_exporter.construct.assets.component import Component

from azura_chat_exporter.ext.cache import clear_cache
from azura_chat_exporter.parse.mention import pass_bot
...
def toKoreaDateStr(str):
        str = str.replace("AM", "오전").replace("PM", "오후")
        ...
        for en, kr in weekdays.items():
            str = str.replace(en, kr)
...
class TranscriptDAO:
    ...
    async def export_transcript(self, message_html: str, meta_data: str):
        ...
        if self.military_time:
            time_now = datetime.datetime.now(timezone).strftime("%Y년 %m월 %d일 %A %p %I:%M")
        else:
        ...
        self.html = await fill_out(self.channel.guild, total, [
            ...
            ("DATE_TIME", toKoreaDateStr(str(time_now))),
...
class Transcript(TranscriptDAO):
    async def export(self):
        ...
        try:
            ...
            self.html = "무언가가 잘못되었습니다!"
            traceback.print_exc()
            print("Please send a screenshot of the above error to https://www.github.com/stealkkak/DiscordChatExporterPy")
            return self
