...
def _set_edit_at(message_edited_at):
    return f'<span class="chatlog__reference-edited-timestamp" data-timestamp="{message_edited_at}">(수정됨)</span>'
...
class MessageConstruct:
    ...
    def __init__(
        ...
        self.time_format = "%Y년 %m월 %d일 %A %p %I:%M"
    ...
    def to_local_time_str(self, time):
        ...
        local_time = time.astimezone(timezone("Asia/Seoul"))

        time_str = local_time.strftime("%Y년 %m월 %d일 %A %p %I:%M")
        ...
        for en, kr in weekdays.items():
            time_str = time_str.replace(en, kr)
